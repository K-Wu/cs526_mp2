...
; RUN: opt < %s -load %S/../build/pass/libSLPVectorizer-kdw.so -slpvect-kdw -S -mtriple=x86_64-unknown-linux -march=core-avx2 | FileCheck %s
; RUN: opt < %s -load %S/../build/pass/libSLPVectorizer-kdw.so -slpvect-kdw -S -mtriple=x86_64-unknown-linux -march=core-avx2 -slp-threshold=-1 -slp-vectorize-hor-store | FileCheck %s --check-prefix=THRESH1
; RUN: opt < %s -load %S/../build/pass/libSLPVectorizer-kdw.so -slpvect-kdw -S -mtriple=x86_64-unknown-linux -march=core-avx2 -slp-threshold=-2 -slp-vectorize-hor-store | FileCheck %s --check-prefix=THRESH2
