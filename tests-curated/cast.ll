...
; RUN: opt < %s -load %S/../build/pass/libSLPVectorizer-kdw.so -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7 -basicaa -slpvect-kdw -dce -S | FileCheck %s --check-prefixes=CHECK,SSE42
; RUN: opt < %s -load %S/../build/pass/libSLPVectorizer-kdw.so -mtriple=x86_64-apple-macosx10.8.0 -mcpu=corei7-avx -basicaa -slpvect-kdw -dce -S | FileCheck %s --check-prefixes=CHECK,AVX
