...
; RUN: opt < %s -load %S/../build/pass/libSLPVectorizer-kdw.so -mattr=sse2 -slpvect-kdw -S | FileCheck %s --check-prefix=CHECK --check-prefix=SSE
; RUN: opt < %s -load %S/../build/pass/libSLPVectorizer-kdw.so -mattr=avx2 -slpvect-kdw -S | FileCheck %s --check-prefix=CHECK --check-prefix=AVX
